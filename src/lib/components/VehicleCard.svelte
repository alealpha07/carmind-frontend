			...
			{#if (!vehicleImageUrl || vehicleImageUrl == '') && loaded}
				<div style="width: 100%; position:relative;">
					...
					<button
						...
						onclick={clickAddImage}><span class="mdi mdi--image-plus"></span></button
					>
				</div>
			{:else}
				<div style="width: 100%; position:relative;">
					...
					<button
						...
						onclick={clickEditImage}><span class="mdi mdi--image-edit"></span></button
					>
				</div>
			{/if}
			<p>{data.type}</p>
		...
		<div class="col-6 right-col">
			...
			<div class="row">
				{#if data.isInsured}<p>{formatDate(data.endDateInsurance)}</p>
					<span class="mdi mdi--stopwatch{insuranceIcon}"></span>{:else}
			...
			<div class="row">
				{#if data.hasBill}<p>{formatDate(data.endDateBill)}</p>
					<span class="mdi mdi--stopwatch{billIcon}"></span>{:else}
			...
			<div class="row">
				...
				<span class="mdi mdi--stopwatch{revisionIcon}"></span>
			</div>
		</div>
	</div>
	<div class="row">

<script lang="ts">
    ...
    function showAddVehicle() {
		formData = {
            type: '', brand: '', model: '', 
            registrationYear: '', plateNumber: '', 
            isInsured: false, startDateInsurance: null, 
            endDateInsurance: null, hasBill: false, 
            endDateBill: null, endDateRevision: null
        };
		formFields = [
	...
	function confirmAddVehicle(result: any) {

		VehicleService.addVehicle(result.type, result.brand, result.model, 
		Number(result.registrationYear), result.plateNumber, result.isInsured, 
		new Date(result.startDateInsurance), new Date(result.endDateInsurance), result.hasBill, 
		new Date(result.endDateBill), new Date(result.endDateRevision)).then((res) => {
			//TODO RELOAD VEHICLE
